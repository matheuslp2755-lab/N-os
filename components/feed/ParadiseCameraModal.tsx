    ...
    const applyAnalogAesthetics = (ctx: CanvasRenderingContext2D, w: number, h: number, p: any) => {
        ...
        switch (p.activeVibe) {
            case 'cybershot':
                ...
                contrast = 1.15;
                brightness += 0.05;
                tintColor = "rgba(0, 100, 255, 0.03)"; // Tom digital frio sutil
                tintOpacity = 0.05;
                blur = Math.max(0.1, blur - 0.2); // Mais nítida que analógica
                break;
            case 'memory':
                saturation = 0.8;
                contrast = 0.9;
        ...
        // Interface Sony Cyber-shot HX200V Style overlay
        if (p.activeVibe === 'cybershot') {
            ctx.font = 'bold 24px sans-serif';
            ctx.fillStyle = 'rgba(255,255,255,0.7)';
            ctx.fillText('Sony Cyber-shot', 60, h - 80);
            ctx.font = '18px sans-serif';
            ctx.fillText('DSC-HX200V', 60, h - 50);
            
            // Bateria e ícones
            ctx.strokeStyle = 'white';
            ctx.strokeRect(w - 120, 60, 60, 30);
            ctx.fillRect(w - 115, 65, 30, 20);
        }
        ...
        <div className="fixed inset-0 z-[600] bg-black flex flex-col animate-fade-in overflow-hidden touch-none font-sans">
            ...
            <header className="absolute top-0 left-0 right-0 p-6 flex justify-between items-start z-50 pointer-events-none">
                <div className="pointer-events-auto">
                    <button onClick={onClose} className="w-12 h-12 flex items-center justify-center bg-black/40 backdrop-blur-xl rounded-full border border-white/10 text-white active:scale-90 transition-all shadow-xl">&times;</button>
                </div>
                
                <div className="flex flex-col items-center opacity-80 pointer-events-none">
                    <div className="flex items-center gap-2 bg-black/60 backdrop-blur-md px-4 py-1.5 rounded-full border border-white/10 shadow-2xl">
                        <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse shadow-[0_0_10px_#ef4444]"></div>
                        <span className="text-[10px] text-white font-black uppercase tracking-[0.3em]">Paradise Engine</span>
                    </div>
                </div>

                <div className="flex flex-col gap-4 pointer-events-auto">
                    <button onClick={() => setFacingMode(p => p === 'user' ? 'environment' : 'user')} className="w-12 h-12 flex items-center justify-center bg-black/40 backdrop-blur-xl rounded-full border border-white/10 text-white active:rotate-180 transition-all duration-500 shadow-xl">
            ...
            <footer className="bg-zinc-950 pt-4 pb-12 px-6 z-50 flex flex-col items-center gap-6 border-t border-white/5 relative">
                ...
                {!capturedImage ? (
                    <>
                        {/* Painel de Ajustes Inferior (Edição) */}
                        {showAdjustments && (
                            <div className="w-full bg-zinc-900/80 backdrop-blur-3xl p-6 rounded-[2.5rem] border border-white/10 animate-slide-up shadow-2xl mb-2">
                                <div className="grid grid-cols-2 gap-x-8 gap-y-6">
                                    <div className="space-y-3">
                                        <div className="flex justify-between items-center">
                                            <span className="text-[9px] font-black text-white/50 uppercase tracking-widest">Brilho</span>
                                            <span className="text-[10px] font-mono text-sky-400">{(light * 100).toFixed(0)}%</span>
                                        </div>
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex justify-between items-center">
                                            <span className="text-[9px] font-black text-white/50 uppercase tracking-widest">Matiz</span>
                                            <span className="text-[10px] font-mono text-sky-400">{color}°</span>
                                        </div>
                                        <input type="range" min="-180" max="180" step="1" value={color} onChange={e => setColor(parseFloat(e.target.value))} className="accent-sky-400 h-1 w-full appearance-none bg-white/10 rounded-full" />
                                    </div>
                                    <div className="space-y-3">
                                        <div className="flex justify-between items-center">
                                            <span className="text-[9px] font-black text-white/50 uppercase tracking-widest">Ruído</span>
                                            <span className="text-[10px] font-mono text-sky-400">{(grain * 100).toFixed(0)}%</span>
                                        </div>
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex justify-between items-center">
                                            <span className="text-[9px] font-black text-white/50 uppercase tracking-widest">Foco Manual</span>
                                            <span className="text-[10px] font-mono text-sky-400">{focus.toFixed(1)}m</span>
                                        </div>
                                        <input type="range" min="0" max="10" step="0.1" value={focus} onChange={e => setFocus(parseFloat(e.target.value))} className="accent-white/20 h-1 w-full appearance-none bg-white/10 rounded-full" />
                                    </div>
                                </div>
                            </div>
                        )}
                                ...
                                {VIBES.map((v) => (
                                    ...
                                    >
                                        <div className={`w-20 h-24 rounded-[2.2rem] flex flex-col items-center justify-center border-2 transition-all ${activeVibe === v.id ? 'bg-zinc-900 border-white shadow-[0_0_40px_rgba(255,255,255,0.15)]' : 'bg-zinc-900/50 border-white/5'}`}>
                                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-zinc-700 to-black flex items-center justify-center shadow-inner">
                                                {v.id === 'cybershot' ? (
                                                    <svg className="w-6 h-6 text-sky-400" fill="currentColor" viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5z"/></svg>
                                                ) : (
                                        ...
                                        <div className="text-center">
                                            ...
                                            <span className="text-[7px] text-zinc-600 font-bold uppercase tracking-tight">{v.sub}</span>
                                        </div>
                                    </button>
                                ))}
                            </div>
                        ...
                        <div className="relative mt-2">
                            <button 
                                ...
                                className="w-24 h-24 rounded-full border-2 border-white/20 flex items-center justify-center p-2 group active:scale-95 transition-all shadow-[0_0_50px_rgba(255,255,255,0.1)]"
                            >
                    ...
                    <div className="flex gap-4 w-full max-w-sm animate-slide-up py-4">
                        ...
                        <a 
                            href={capturedImage} 
                            download={`paradise-${Date.now()}.jpg`} 
                            className="flex-1 py-5 bg-white text-black text-[10px] font-black uppercase tracking-[0.3em] rounded-[2rem] text-center shadow-2xl active:scale-95 transition-all"
                        >
                            Exportar
                        </a>
                    </div>
